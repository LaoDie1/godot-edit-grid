...
var _control_highlighter : Control = null
...
func _update_branch(root, root_view: TreeItem):
	...
	for i in root.get_child_count():
		...
		if i >= children_views.size():
			...
			if child.name != child_view_name:
				_node_changed_view(child, child_view)
...
func _create_node_view(node, parent_view):
	...
	view.collapsed = true
	_node_changed_view(node, view)
	return view


func _node_changed_view(node, view):
	assert(node is Node)
